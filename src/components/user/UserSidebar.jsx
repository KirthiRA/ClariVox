...
import {
  ...
  FolderOpen, Settings, User, ChevronDown,
  Crown, CheckCircle, LogOut
} from 'lucide-react'
...
export default function UserSidebar() {
  ...
  const Expander = ({ open, toggle, icon: Icon, label }) => (
    <button onClick={toggle} style={{ ...ls(false), justifyContent:'space-between', background:'transparent', border:'none' }}>
      <span style={{ display:'flex', alignItems:'center', gap:12 }}><Icon size={17}/>{label}</span>
  ...
  return (
    <aside style={{
      width:258, background:'#0c0e1a',
      borderRight:'1px solid rgba(91,141,238,0.12)',
      ...
      <div style={{ padding:'20px 18px', borderBottom:'1px solid rgba(91,141,238,0.12)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            ...
            <div style={{ color:'#3d4a6b', fontSize:10, marginTop:1 }}>AI Voice Intelligence</div>
          </div>
        </div>
      </div>
      ...
      <nav style={{ flex:1, padding:'14px 10px', overflowY:'auto' }}>
        ...
        <Expander open={meetOpen} toggle={()=>setMeetOpen(p=>!p)} icon={FileText} label="Meetings"/>
        {meetOpen && <div style={{ paddingLeft:16 }}>
          <Link to="/meetings" style={ls(isActive('/meetings'))}>Meeting History</Link>
          <Link to="/meetings" style={ls(false)}>Archived</Link>
        </div>}

        <Expander open={sumOpen} toggle={()=>setSumOpen(p=>!p)} icon={BookOpen} label="Summaries"/>
        {sumOpen && <div style={{ paddingLeft:16 }}>
          <Link to="/meetings" style={ls(false)}>All Summaries</Link>
          <Link to="/meetings" style={ls(false)}>Favourites</Link>
        </div>}

        <Expander open={projOpen} toggle={()=>setProjOpen(p=>!p)} icon={FolderOpen} label="Project"/>
        {projOpen && <div style={{ paddingLeft:16 }}>
          <Link to="/dashboard" style={ls(false)}>My Projects</Link>
          <Link to="/dashboard" style={ls(false)}>Shared</Link>
        </div>}
